...
## tab3 callbacks
@app.callback(Output('barh-Store_type_country','figure'),
            [Input('Store_type_dropdown','value')])
def tab3_barh_store_subcat(chosen_type):

    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['Store_type']==chosen_type)].pivot_table(index='country',columns='Gender',values='total_amt',aggfunc='sum').assign(_sum=lambda x: x['F']+x['M']).sort_values(by='_sum').round(2)
...
@app.callback(Output('pie-Store_type_day','figure'),
            [Input('Weekday_dropdown','value')])
def tab3_pie_store(chosen_type):
    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['weekday']==chosen_type)].groupby('Store_type')['total_amt'].sum()

    fig = go.Figure(data=[go.Pie(labels=grouped.index,values=grouped.values, pull=[0.05, 0.05, 0.05, 0.1])],layout=go.Layout(title='Udział rodzaju sklepu w sprzedaży'))
    return fig
